...
## 2026-07-06 Frontend Release Deployment Guard

- Added a guarded frontend release deployment executor:
  - `tools/deploy_rehab_mobile_frontend_release.py`
  - `tools/test_deploy_rehab_mobile_frontend_release.py`
- Updated the release manifest generated by
  `tools/prepare_rehab_mobile_frontend_release.py` with `deploy.executor_command`.
- Updated `tools/verify_rehab_mobile_frontend_release.py` so
  `FRONTEND-RELEASE-DEPLOYMENT` requires the executor command before cloud copy.
- Updated the Stitch repair packet, V4 prompt, runbook, scorecard, and QA report
  so the normal post-Stitch path is:
  1. local frontend L1 preflight,
  2. release bundle packaging,
  3. release manifest verification,
  4. deployer dry-run,
  5. deployer `--execute --run-post-verify`.
- The deployer refuses unsafe remote roots and defaults to dry-run.
- No cloud runtime deployment was made for this tooling-only change; current
  runtime remains build SHA `d2f81c92`, PID `2052592`.
