...
| Stitch MCP generation attempt | FAIL, OUTPUT REJECTED | Stitch MCP endpoint is reachable and project `projects/1542023501541093471` was created. A focused home resmoke produced screen `bdcea50a83a74eb68075807aaf7120dc`, but Stitch rewrote required `查看康复师建议` / `问康复师` into `开始康复训练` / `咨询治疗师`; no generated HTML was copied into frontend source |
