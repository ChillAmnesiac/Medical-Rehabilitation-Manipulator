...
- Source baseline: formal `M33@24bae363`; the reviewable EMG + BLE integration branch is `WEN/m33-emg-ble-integrated-20260712`.
...
- A clean GCC 13.3/SCons build produces the expected size `text=774412 data=2776 bss=324445`.
- Do not flash a raw HEX beginning with `:020000040834BE`. Run the repository Edge Protect relocation/merge step and require `build/rtthread.hex` to begin with `:02000004603466`.
- HEX SHA256 can differ between worktree paths because absolute build paths are embedded in the image. Compare the Git tree, size, relocated address, and board QA results together; do not use a cross-worktree hash as the only equivalence check.
