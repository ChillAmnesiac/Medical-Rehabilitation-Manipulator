...
Read the latest completed turn:

```text
m55qa_xz_latency
```

Run 20 text-only LLM + TTS turns from the M33 worktree:

```powershell
python tools\xiaozhi_latency_benchmark.py --port COM26 --mode text --iterations 20
```

Run 20 deterministic PCM QA turns. This mode automatically enables the QA gate, starts capture, sends the built-in 1200 ms PCM sample, stops capture, and disables the gate even after failure:

```powershell
python tools\xiaozhi_latency_benchmark.py --port COM26 --mode probe --probe-ms 1200 --iterations 20
```

Observe 30 real wake/mic turns without injecting commands:

```powershell
python tools\xiaozhi_latency_benchmark.py --port COM26 --mode observe --iterations 30 --timeout 45
```

Outputs are `turns.csv` and `report.json`. Flags identify real wake (`0x3`), manual/probe (`0x5`), or QA text (`0x9`). `speech_audio` is the product metric for real voice; use `stop_audio` for text QA. Pass criteria are p50 <= 3000 ms, p95 <= 5000 ms, at least 30% improvement from baseline, and zero truncation, timeout, queue drop, underrun, or fault.

Safety: do not run motor enable, position, speed, torque, trajectory, or motion commands. Do not run `m33_cm55_restart` while BLE is active.
