...
static void test_expiry_latches_and_late_heartbeat_does_not_cancel_stop(void)
{
    ...
    rehab_can_lease_note_mode(&lease, RT_TRUE, 7U);
    assert(rehab_can_lease_claim_stop(&lease, 600U, 500U, 100U, &generation) == RT_FALSE);
    assert(rehab_can_lease_claim_stop(&lease, 601U, 500U, 100U, &generation) == RT_TRUE);
    assert(generation == 7U);

    rehab_can_lease_note_heartbeat(&lease, 602U);
    assert(rehab_can_lease_claim_stop(&lease, 603U, 500U, 100U, &generation) == RT_FALSE);
    assert(rehab_can_lease_claim_stop(&lease, 701U, 500U, 100U, &generation) == RT_TRUE);
}
...
static void test_failed_stop_remains_latched_for_retry(void)
{
    ...
    rehab_can_lease_note_mode(&lease, RT_TRUE, 3U);
    assert(rehab_can_lease_claim_stop(&lease, 100U, 50U, 20U, &generation) == RT_TRUE);
    rehab_can_lease_note_stop_result(&lease, RT_FALSE, RT_FALSE);

    assert(lease.stop_retry_count == 1U);
    assert(rehab_can_lease_claim_stop(&lease, 101U, 50U, 20U, &generation) == RT_FALSE);
    assert(rehab_can_lease_claim_stop(&lease, 120U, 50U, 20U, &generation) == RT_TRUE);
...
static void test_success_or_ownership_change_releases_old_lease(void)
{
    ...
    rehab_can_lease_note_mode(&lease, RT_TRUE, 4U);
...
static void test_tick_wrap_uses_unsigned_age(void)
{
    ...
    rehab_can_lease_note_mode(&lease, RT_TRUE, 9U);
    assert(rehab_can_lease_claim_stop(&lease, 29U, 50U, 20U, &generation) == RT_FALSE);
    assert(rehab_can_lease_claim_stop(&lease, 30U, 50U, 20U, &generation) == RT_TRUE);
}
